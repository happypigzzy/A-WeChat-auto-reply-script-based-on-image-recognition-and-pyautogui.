...
detection_area = (2087, 990, 100, 100)
...
def main():
    ...
    try:
        while True:
            red_dots = detect_red_dots()

            if red_dots:
                print(f"检测到 {len(red_dots)} 个未读消息")

                # 处理所有红点
                for i, (x, y, area) in enumerate(red_dots):
                    print(f"处理第 {i + 1} 个未读消息 (位置: {x}, {y}, 面积: {area})")
